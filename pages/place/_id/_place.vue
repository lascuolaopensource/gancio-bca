<template>
  <v-container id="home" class="px-2 px-sm-6 pt-0">
    <h1
      class="d-block text-h4 font-weight-black text-center text-uppercase mt-10 mx-auto w-100 text-underline"
    >
      <u>{{ place.name }}</u>
    </h1>
    <span
      v-if="place.name !== 'online'"
      class="d-block text-subtitle text-center w-100"
      >{{ place.address }}</span
    >
    ...
    <div v-if="settings.allow_geolocation && place.latitude && place.longitude">
      <div class="mt-4 mx-n2 md:mx-0">
        <Map
          :place="place"
          :LMapStyles="{ 'border-radius': 0, height: '16rem' }"
        />
      </div>
      <div class="mt-4">
...
export default {
  ...
  async asyncData({ $axios, params, error }) {
    try {
      const { events, pastEvents, place } = await $axios.$get(
        `/place/${params.id}`
      )
