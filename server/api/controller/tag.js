...
module.exports = {
  async _findOrCreate(tags) {
    ...
    const existingTags = await sequelize.query(
      `SELECT * FROM ${escapeCol('tags')} where LOWER(${escapeCol(
        'tag'
      )}) in (${tags.map((t) => 'LOWER(?)').join(',')})`,
      {
        model: Tag,
        mapToModel: true,
        replacements: trimmedTags,
        type: Sequelize.QueryTypes.SELECT
      }
    )
    const lowercaseExistingTags = existingTags.map((t) =>
      t.tag.toLocaleLowerCase()
    )

    const remainingTags = uniqBy(
      trimmedTags.filter(
        (t) => !lowercaseExistingTags.includes(t.toLocaleLowerCase())
      ),
      toLower
    )
  ...
  async getEvents(req, res) {
    ...
    switch (format) {
      case 'rss':
        return exportController.feed(
          req,
          res,
          events,
          `${res.locals.settings.title} - Tag #${tag}`,
          `${res.locals.settings.baseurl}/feed/rss/tag/${encodeURIComponent(
            tag
          )}`
        )
      case 'ics':
      ...
      default:
        const pastEvents = await eventController._select({
          tags: tag,
          show_recurrent: true,
          show_multidate: true,
          show_federated: true,
  ...
  async updateTag(req, res) {
    try {
      ...
      if (!newtag) {
        ...
        await Tag.update(
          { tag: req.body.newTag },
          { where: { tag: req.body.tag }, raw: true }
        )
      } else {
        // in case it exists:
