<template>
  <v-card class="eventModeration">
    <v-card-title
      >{{ $t('common.moderation') }} <v-spacer /><v-btn
        text
        label
        icon
        @click="$emit('close', false)"
        size="small"
        ><v-icon v-text="mdiChevronRight" /></v-btn
    ></v-card-title>
    <v-card-text class="d-flex flex-column flex-grow-1 overflow-auto">
      <v-textarea
        :label="$t('event.message')"
        :hint="$t(isAdmin ? 'event.message_hint' : 'event.message_author_hint')"
        persistent-hint
        v-model="message"
        rows="2"
        class="mb-2"
      ...
      <template v-if="isAdmin">
        ...
        <v-btn
          ...
          color="primary"
          >{{ $t('event.send_to_author_too') }}</v-btn
        >
      </template>
      <v-btn
        v-else
        ...
        >
          <v-list-item-content>
            ...
            <v-list-item-subtitle class="font-weight-light"
              >{{ $time.format(item.createdAt, 'EEEE d MMMM HH:mm') }} /
